...
class VerificationManager:
    """Manages file integrity and system verification."""
    ...
    def calculate_checksum(file_path: str) -> str:
        ...
        try:
            ...
            logging.error(f"Checksum calculation failed for {file_path}: {e}")
            return ""

    @staticmethod
    def verify_project_integrity(root_dir: str) -> Dict[str, Any]:
        """Verify the integrity of the project files."""
        results = {"success": True, "errors": [], "checked_files": 0}
        
        # Critical files that must exist and be valid python
        critical_files = [
            "__main__.py", 
        ...
        for rel_path in critical_files:
            ...
            if not os.path.exists(file_path):
                ...
                results["errors"].append(f"Missing critical file: {rel_path}")
                continue
            ...
            try:
                ...
                results["success"] = False
                results["errors"].append(f"Syntax error in {rel_path}: {e}")

        return results


class SelfImprovementPermission:
    """Manages explicit permissions for self-improvement."""
    
    def __init__(self, state_file: str = "permission.json"):
        ...
        if os.path.exists(self.state_file):
            try:
                with open(self.state_file, "r") as f:
                    data = json.load(f)
                    self.status = data.get("status", "DENIED")
            except:
                self.status = "DENIED"
    ...
    def _save_state(self):
        with open(self.state_file, "w") as f:
            json.dump({"status": self.status, "timestamp": datetime.now().isoformat()}, f)

    def is_granted(self) -> bool:
        return self.status == "CONFIRMED"


def rollback_to_stable(backup_dir: str, target_dir: str) -> bool:
    """Rollback system to the last stable state from backup."""
    import shutil
    print(f"(!) INITIATING ROLLBACK from {backup_dir} to {target_dir}")
    try:
        # 1. Verify backup exists
        if not os.path.exists(backup_dir):
            logging.critical("Rollback failed: Backup directory not found!")
            return False

        # 2. Clear target directory (exclude .git and protected data if needed, simplified here)
        # For safety, we iterate and remove only files/folders that overwrite, or clean specific paths.
        # Here we assume a full restore is desired.
