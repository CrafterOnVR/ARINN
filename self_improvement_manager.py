...
class SelfImprovementManager:
    ...
    def initiate_self_improvement(self, research_topic: str, research_results: Dict[str, Any]) -> Dict[str, Any]:
        """Initiate the self-improvement cycle with STRICT safety protocols."""
        ...
        permission = SelfImprovementPermission()
        if not permission.is_granted():
            print("[ARINN-LOG] SELF-IMPROVEMENT BLOCKED: Permission DENIED (Change 'status' in permission.json to 'CONFIRMED')")
            return result
        ...
        try:
            ...
            print(f"[ARINN-LOG] Creating Safety Snapshot at {backup_dir}...")
            shutil.copytree(project_dir, backup_dir, ignore=shutil.ignore_patterns('__pycache__', '*.pyc', 'backup_*', '.git'))

            # Step 3: CRUCIBLE GATEKEEPER & APPLY CHANGES
            ...
            def llm_generator(prompt: str) -> str:
                import llm
                client = llm.LLMClient(enabled=True)
                resp = client._handle_api_call(
            ...
            for imp in improvements:
                ...
                if not passed:
                    ...
                    print(f"[CRUCIBLE] RSI Mutation Passed Sandbox. Saving golden weights.")
                    # In a full run, we would apply the AST here. 
            
            if not update_result["success"]:
                print(f"[ARINN-LOG] Crucible blocked mutation: {update_result.get('error')}")
                raise Exception(f"Code application failed: {update_result.get('error')}")

            # Step 4: VERIFICATION SUITE
            print("[ARINN-LOG] Running Integrity Verification...")
            integrity = VerificationManager.verify_project_integrity(project_dir)
            
            if not integrity["success"]:
                print(f"[ARINN-LOG] VERIFICATION FAILED: {integrity['errors']}")
                # Step 5: ROLLBACK
                print("[ARINN-LOG] INITIATING AUTOMATIC ROLLBACK...")
                rollback_success = rollback_to_stable(backup_dir, project_dir)
                result["rollback_triggered"] = True
                if rollback_success:
                    print("[ARINN-LOG] System restored to stable state.")
                    return result
                else:
                    print("[ARINN-LOG] CRITICAL: Rollback failed. Manual intervention required.")
                    result["errors"].append("Critical Rollback Failure")
                    return result
            
            # Step 6: SUCCESS & PERMISSION REVOCATION
            print("[ARINN-LOG] Self-improvement verified successful.")
            permission.revoke() # Revoke permission requiring re-confirmation for next time
            print("[ARINN-LOG] Permission successfully revoked (User must re-confirm for next cycle).")
            ...
            if 'backup_dir' in locals() and os.path.exists(backup_dir):
                 rollback_to_stable(backup_dir, project_dir)
                 result["rollback_triggered"] = True
            return result
