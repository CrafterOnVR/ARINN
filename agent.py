...
class ResearchAgent:
            ...
            try:
                ...
                for idx, result in enumerate(fetched_results):
                    if isinstance(result, Exception):
                        continue # Ignore individual fetch crashes
                    url, text = result
                    if text:
                        added, doc_id = self.db.add_document(topic_id, url, "", text, created_at=utc_now_iso())
                        if added:
                ...
                # Explicit Memory Optimization for 168-Hour Cycle
                import gc
                gc.collect()
                try:
                ...
                print(f"[CRITICAL ERROR] Execution swept prevented loop crash: {e}. Suspending 15 seconds.")
    ...
    async def _ensure_questions(self, topic_id: int, topic_name: str, regen: bool = False, target: int = 40):
        ...
        if self.llm.enabled:
            try:
                ...
                print(f"[!] Teacher API generation failed ({e}). Falling back to heuristics.")
                
        # Pure autonomous fallback if the Teacher API (OpenRouter) is physically unavailable or returned empty bounds
        if not questions:
            print(f"[!] No valid questions returned from Teacher. Activating physical heuristic generation.")
    ...
    def _heuristic_questions(self, topic: str, context: str, target: int) -> List[str]:
        # Strip long punctuation that might confuse search engine keyword parsing
        clean_topic = topic.replace(",", "").replace("&", "and")
        
        # Use concise keyword-based queries rather than full conversational sentences
        seeds = [
            f"{clean_topic} core concepts and fundamentals",
            f"{clean_topic} practical application examples",
            f"{clean_topic} common misconceptions",
            f"{clean_topic} popular tools and libraries",
            f"{clean_topic} historical milestones",
            f"{clean_topic} pros and cons tradeoffs",
            f"{clean_topic} comparison to alternatives",
            f"{clean_topic} best learning resources",
            f"{clean_topic} advanced techniques",
            f"{clean_topic} modern industry trends",
        ]
        
        extras = [f"{clean_topic} beginner tutorial", f"{clean_topic} expert guide", f"{clean_topic} case studies"]
        out = seeds + extras
        return out[:target]
