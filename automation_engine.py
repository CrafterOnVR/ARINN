...
class TaskForecaster:
    """
    Predicts future tasks based on historical sequences.
    Phase 6: Advanced Autonomy Enhancement.
    """
    def __init__(self):
        # Maps current_task_type -> Counter(next_task_type)
        self.transitions = defaultdict(Counter)
        self.last_task_type = None

    def record_transition(self, task_type: str):
        """Learns the sequence: last_task -> current_task"""
        if self.last_task_type:
            self.transitions[self.last_task_type][task_type] += 1
        self.last_task_type = task_type

    def predict_next(self, current_task_type: str) -> List[Tuple[str, float]]:
        """Returns list of (next_task_type, probability)"""
        if current_task_type not in self.transitions:
            return []
        
        counts = self.transitions[current_task_type]
        total = sum(counts.values())
        if total == 0:
            return []
            
        predictions = []
        for next_type, count in counts.items():
            prob = count / total
            predictions.append((next_type, prob))
...
class AdvancedAutomationEngine:
    ...
    def __init__(self, max_workers: int = 5):
        ...
        self.forecaster = TaskForecaster() # Phase 6 Enhancement
    ...
    async def _worker_loop(self):
        ...
        while self.running:
            try:
                ...
                # Phase 6: Forecast next steps
                predictions = self.forecaster.predict_next(task.task_type)
                if predictions:
                    top_pred, prob = predictions[0]
                    if prob > 0.8:
                        logging.info(f"[FORECAST] High probability ({prob:.1%}) that '{top_pred}' follows '{task.task_type}'.")
                
                # Execute task
                await self._execute_task(task)
                
                # Phase 6: Learn sequence
                if task.status == "completed":
                    self.forecaster.record_transition(task.task_type)
