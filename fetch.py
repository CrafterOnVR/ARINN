...
class Fetcher:
    ...
    async def fetch_text(self, url: str, use_proxy: bool = True, custom_headers: Optional[Dict[str, str]] = None) -> Optional[str]:
        ...
        try:
            ...
            if proxies is None:
                resp = await self.client.get(url, timeout=self.timeout, follow_redirects=True, headers=headers)
            else:
                async with httpx.AsyncClient(proxy=proxies, transport=httpx.AsyncHTTPTransport(retries=3)) as client:
